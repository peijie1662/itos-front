<template>
  <div>
    <el-dialog title="登记服务" width="600px" :visible.sync="dialogVisible">
      <el-form ref="form" :model="app" label-width="120px">
        ...
        <!-- 按钮 -->
        <el-form-item>
          <el-button type="primary" @click="saveApp" size="mini" style="margin-left:100px;">立即创建</el-button>
...
import { addAppInfo } from "@/api/api";
    ...
    saveApp() {
      ...
      me.$refs.form.validate(valid => {
        if (valid) {
          ...
          addAppInfo(me.app).then(res => {
            let { flag, errMsg } = res;
            if (!flag) {
              me.$message.error(errMsg);
            } else {
              me.$message.success("添加服务信息成功");
